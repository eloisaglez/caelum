...
El archivo CSV viene de la **tarjeta MicroSD** del CanSat (ya limpio con `limpiar_espera.py`) o del backup de RAM. Tiene 27 columnas:

```csv
num_paquete,equipo,timestamp,datetime,lat,lon,alt,alt_mar,sats,temp_hs,hum_hs,temp_scd,hum_scd,temp_lps,presion,co2,pm1_0,pm2_5,pm10,accel_x,accel_y,accel_z,gyro_x,gyro_y,gyro_z,fase
1,CAELUM,0,2026-03-17T11:30:00,40.405200,-3.993100,998.2,1648.2,9,5.8,68.1,5.9,67.0,6.2,901.2,421,4.1,6.2,9.8,0.05,-0.02,9.80,1.2,-0.5,0.3,caida_libre
```
...
**CO₂ — confirmación de sensor** (~420 ppm constante en vuelo normal). No indica fuentes de combustión a esta altitud.

---

## Scripts

### limpiar_espera.py — PASO PREVIO (obligatorio para datos_SD_raw.csv)

**Función:** El Arduino guarda todos los datos en `datos_SD_raw.csv` incluyendo la fase `espera` (antes del lanzamiento y durante la subida en globo). Este script elimina esas filas y genera `datos_SD.csv` listo para analizar.
...
> No es necesario para `datos_RAM.csv` ni `datos_simulacion.csv` — ya contienen solo datos de vuelo.
...
El script acepta cualquier CSV con las 27 columnas, independientemente del origen:

| Fichero | Origen | Cuándo usar |
|---------|--------|-------------|
| `datos_SD_raw.csv` | Tarjeta MicroSD (datos crudos) | Requiere limpiar_espera.py primero |
| `datos_SD.csv` | Generado por limpiar_espera.py | **Fuente principal — máxima resolución** |
...
| `datos_simulacion.csv` | Simulador (ver `software/simulacion/`) | Pruebas pre-vuelo |
...
1. **Limpiar datos crudos** (solo para datos_SD_raw.csv):
```bash
python limpiar_espera.py datos_SD_raw.csv
```
...
──── CO₂ (Confirmacion de sensor SCD40) ────
  Media: 449 ppm  |  rango: 115 ppm
  → REVISAR SENSOR (Δ = 115 ppm > 30 ppm) — no indica combustion a esta altitud ⚠️
...
**Proyecto:** CanSat Misión 2 — Análisis de Capas Atmosféricas y Perfiles Verticales de Partículas
**Fecha:** Febrero 2026
