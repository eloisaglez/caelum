...
OBJETIVOS CIENTÍFICOS:
  ...
  2. CO₂ como confirmación de funcionamiento del sensor (T+HR son los datos valiosos del SCD40)
...
#  CONFIGURACIÓN
# ──────────────────────────────────────────────────────────────
INPUT_FILE  = None                 # Pasar como argumento: python analizar_vuelo.py datos_SD.csv
...
UMBRAL_CO2_VARIACION   = 30.0   # ppm — por encima de esto posible ruido de sensor (precisión SCD40 = ±10 ppm)
...
def cargar_datos(filepath):
    """Carga el CSV, limpia datos inválidos y detecta sensores disponibles."""
    ...
    # ── Detectar sensores con datos válidos (>10% de filas con valor != 0) ──
    sensores = {
        'pm':    ['pm1_0', 'pm2_5', 'pm10'],
        'co2':   ['co2'],
        'hs300x':['temp_hs', 'hum_hs'],
        'scd40': ['temp_scd', 'hum_scd'],
        'lps':   ['presion', 'temp_lps'],
        'imu':   ['accel_x', 'accel_y', 'accel_z'],
        'gps':   ['lat', 'lon'],
    }
    disponible = {}
    for sensor, cols in sensores.items():
        cols_ok = [c for c in cols if c in df.columns]
        if not cols_ok:
            disponible[sensor] = False
            continue
        # Sensor disponible si al menos una columna tiene >10% de filas con valor != 0
        pct_valido = max((df[c] != 0).mean() for c in cols_ok)
        disponible[sensor] = pct_valido > 0.10

    print("   Sensores detectados:")
    for sensor, ok in disponible.items():
        estado = "✅" if ok else "❌ SIN DATOS"
        print(f"     {sensor:<10} {estado}")
    print()

    # ── Convertir ceros a NaN en columnas de sensores opcionales ──
    # Así matplotlib dibuja huecos en vez de líneas en cero
    cols_nan = ['pm1_0', 'pm2_5', 'pm10', 'co2',
                'temp_scd', 'hum_scd', 'temp_lps',
                'accel_x', 'accel_y', 'accel_z']
    for col in cols_nan:
        if col in df.columns:
            df[col] = df[col].replace(0, np.nan)

    # HS300x — temperatura de referencia, si es cero también es error
    for col in ['temp_hs', 'hum_hs']:
        if col in df.columns:
            df[col] = df[col].replace(0, np.nan)

    # Presión cero = sensor LPS no disponible
    if 'presion' in df.columns:
        df['presion'] = df['presion'].replace(0, np.nan)

    # ── GPS ──
    tiene_gps = (df['lat'] != 0) & (df['lon'] != 0)
    n_sin_gps = (~tiene_gps).sum()
    if n_sin_gps > 0:
        print(f"   ⚠️  {n_sin_gps} filas sin fix GPS (no se incluyen en el mapa)")
    ...
    return df, df[tiene_gps].copy(), disponible
...
def detectar_inversiones(df, bin_size=50):
    ...
    # NaN en columnas de sensores ausentes — sustituir por 0 para no romper comparaciones
    perfil['pm25_media']  = perfil['pm25_media'].fillna(0)
    perfil['pm10_media']  = perfil['pm10_media'].fillna(0)
...
def graf_perfil_vertical(df, perfil, outdir):
    ...
    fig, axes = plt.subplots(1, 4, figsize=(13, 6.5), sharey=True)
    ...
    # ── Panel 3: CO₂ — confirmación sensor ──
    ax = axes[2]
    ...
    if not co2_perfil.empty:
        ...
        estado = 'Sensor OK ✓' if rango < UMBRAL_CO2_VARIACION else f'Revisar sensor\n(Δ={rango:.0f} ppm)'
...
def graf_inversiones(df, perfil, outdir):
    ...
    # Resumen dentro del panel 1 (no debajo para evitar solapamiento)
    axes[0].text(0.97, 0.03, resumen, transform=axes[0].transAxes,
...
def generar_mapa(df_gps, perfil, outdir):
    ...
    # Mapa satelital (Esri World Imagery)
    mapa = folium.Map(
        location=centro,
        zoom_start=15,
    ...
    # Trayectoria como segmentos coloreados por PM2.5
    # Cada segmento entre dos puntos consecutivos adopta el color del PM2.5 del punto inicial
    filas = df_gps.reset_index(drop=True)
    for i in range(len(filas) - 1):
        p1  = [filas.loc[i,   'lat'], filas.loc[i,   'lon']]
        p2  = [filas.loc[i+1, 'lat'], filas.loc[i+1, 'lon']]
        pm  = filas.loc[i, 'pm2_5'] if 'pm2_5' in filas.columns else 0
        folium.PolyLine(
            [p1, p2],
            color=color_pm25(pm),
            weight=4,
    ...
    # Puntos cada 5 muestras con popup de datos (no saturar el mapa)
    for i, row in filas.iterrows():
        if i % 5 != 0:
            continue
        popup = (
            f"<b>Altitud:</b> {row['alt']:.0f} m<br>"
            f"<b>PM2.5:</b> {row.get('pm2_5', 0):.1f} µg/m³<br>"
            f"<b>PM10:</b> {row.get('pm10', 0):.1f} µg/m³<br>"
            f"<b>CO₂:</b> {row.get('co2', 0):.0f} ppm<br>"
            f"<b>Temp:</b> {row.get('temp_hs', 0):.1f} °C<br>"
            f"<b>Fase:</b> {row.get('fase', '—')}"
        )
        folium.CircleMarker(
            location=[row['lat'], row['lon']],
            radius=4, color='white', weight=0.5,
            fill=True, fill_color=color_pm25(row.get('pm2_5', 0)), fill_opacity=0.9,
            popup=folium.Popup(popup, max_width=200)
        ).add_to(mapa)

    # Marcadores de fases clave
    # Lanzamiento
    folium.Marker(
        [filas['lat'].iloc[0], filas['lon'].iloc[0]],
        popup=folium.Popup('<b>Lanzamiento</b>', max_width=120),
        icon=folium.Icon(color='green', icon='arrow-up', prefix='fa')
    ).add_to(mapa)

    # Apertura de paracaídas — primer punto con fase 'apertura' o 'descenso'
    df_apertura = filas[filas['fase'].isin(['apertura', 'descenso'])]
    if len(df_apertura) > 0:
        fila_ap = df_apertura.iloc[0]
        popup_ap = (
            f"<b>Apertura paracaidas</b><br>"
            f"Alt: {fila_ap['alt']:.0f} m<br>"
            f"PM2.5: {fila_ap.get('pm2_5', 0):.1f} µg/m³"
        )
        folium.Marker(
            [fila_ap['lat'], fila_ap['lon']],
            popup=folium.Popup(popup_ap, max_width=160),
            icon=folium.Icon(color='blue', icon='flag', prefix='fa')
        ).add_to(mapa)

    # Aterrizaje
    folium.Marker(
        [filas['lat'].iloc[-1], filas['lon'].iloc[-1]],
        popup=folium.Popup('<b>Aterrizaje</b>', max_width=120),
        icon=folium.Icon(color='red', icon='arrow-down', prefix='fa')
    ).add_to(mapa)

    # Leyenda PM2.5
    leyenda = """
    <div style='position:fixed;bottom:30px;left:30px;background:rgba(0,0,0,0.75);
    ...
    <span style='color:#DD0000'>&#9644;</span> &gt;55 µg/m³ — Muy malo<br>
    <br>
    <b>Fases</b><br>
    <span style='color:#00AA00'>&#9650;</span> Lanzamiento<br>
    <span style='color:#4488FF'>&#9679;</span> Apertura paracaidas<br>
    <span style='color:#DD0000'>&#9660;</span> Aterrizaje
    </div>"""
...
def generar_informe(df, perfil, outdir):
    ...
    if co2_rango < UMBRAL_CO2_VARIACION:
        lineas.append(f"  → SENSOR FUNCIONANDO CORRECTAMENTE (Δ < {UMBRAL_CO2_VARIACION} ppm) ✓")
    else:
        lineas.append(f"  → REVISAR SENSOR (Δ = {co2_rango:.0f} ppm > {UMBRAL_CO2_VARIACION} ppm) — no indica combustión a esta altitud ⚠️")
...
if __name__ == "__main__":
    # Permitir pasar el CSV como argumento: python analizar_vuelo.py mi_vuelo.csv
    if len(sys.argv) > 1:
        INPUT_FILE = sys.argv[1]
    else:
        print("❌ Debes indicar el archivo CSV a analizar.")
        print("   Uso: python analizar_vuelo.py <archivo.csv>")
        print("   Ejemplos:")
        print("     python analizar_vuelo.py datos_SD.csv")
        print("     python analizar_vuelo.py datos_RAM.csv")
        print("     python analizar_vuelo.py datos_radio.csv")
        print("     python analizar_vuelo.py datos_simulacion.csv")
        sys.exit(1)
    ...
    df, df_gps, disponible = cargar_datos(INPUT_FILE)
    perfil = detectar_inversiones(df)

    resultados = []

    def ejecutar(nombre, func, *args):
        """Ejecuta una función de análisis y captura errores sin detener el resto."""
        try:
            func(*args)
            resultados.append(f"   ✅ {nombre}")
        except Exception as e:
            resultados.append(f"   ⚠️  {nombre} — omitido ({e})")
            print(f"   ⚠️  {nombre} omitido: {e}")

    ejecutar("graf_1_perfil_vertical.png",     graf_perfil_vertical,   df, perfil, OUTPUT_DIR)
    ejecutar("graf_2_inversiones_termicas.png", graf_inversiones,       df, perfil, OUTPUT_DIR)
    ejecutar("graf_3_validacion_cruzada.png",   graf_validacion_cruzada,df, OUTPUT_DIR)
    ejecutar("graf_4_mision_primaria.png",      graf_mision_primaria,   df, df_gps, OUTPUT_DIR)

    if disponible.get('gps'):
        ejecutar("mapa_vuelo.html",             generar_mapa,           df_gps, perfil, OUTPUT_DIR)
    else:
        print("   ⚠️  mapa_vuelo.html — omitido (sin datos GPS)")
        resultados.append("   ⚠️  mapa_vuelo.html — omitido (sin datos GPS)")

    ejecutar("informe_vuelo.txt",               generar_informe,        df, perfil, OUTPUT_DIR)
