...
"""
════════════════════════════════════════════════════════════════
   CANSAT MISIÓN 2 - SIMULADOR DE VUELO v2
...
#
#  CO₂: trazador de estabilidad → constante en altura (~420 ppm),
#       aumenta gradualmente al acercarse al suelo por fuentes de
#       combustión (tráfico M-501, actividad local Brunete)
#
...
PERFIL_CONTAMINACION = [
    ...
    (700, 1000, 418, 5,  "Troposfera libre — aire limpio bien mezclado"),
    (500, 700,  422, 8,  "Transición — ligera influencia regional"),
    (350, 500,  430, 12, "Tope inversión — inicio acumulación"),
    (200, 350,  445, 48, "INVERSIÓN TÉRMICA — acumulación máxima PM"),
    (100, 200,  470, 30, "Bajo la inversión — gradiente PM"),
    (0,   100,  510, 22, "Capa superficial — tráfico y suelo"),
]
...
def simular_vuelo():
    print("═" * 60)
    print("   🚀 SIMULADOR DE VUELO — CANSAT MISIÓN 2 v2")
    print("═" * 60)
    ...
    print(f"    CO₂ constante en altura → ¿atmósfera mezclada? "
          f"{'Sí ✓' if df['co2'].max()-df['co2'].min() < 40 else 'Capas detectables ⚠️'}")
    ...
    print(f"\n💡 Siguiente paso:")
    print(f"   python analizar_vuelo_v4.py {OUTPUT_FILE}")
    print("\n" + "═" * 60)
