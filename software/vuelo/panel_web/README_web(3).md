...
- `/cansat/replay`: Reproducción de `datos_SD.csv`.
- `/cansat/simulacion`: Datos de `datos_SD.csv`.
...
* **Modo Concurso**: Envía a `/telemetria` y genera automáticamente el archivo `datos_SD.csv`.
...
* **Lógica**: 
    * Si detecta `datos_SD.csv` → Modo **REPLAY**.
    * Si detecta `datos_SD.csv` → Modo **SIMULACIÓN**.
