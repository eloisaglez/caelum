...
#
# RUTAS FIREBASE:
#   datos_SD.csv          → /cansat/replay/    (datos reales — SD card)
#   datos_radio.csv       → /cansat/replay/    (datos reales — telemetría, si no se recuperó el CanSat)
#   datos_simulacion.csv  → /cansat/simulacion/ (datos de simulación)
#
...
CAMPOS_TEXTO = {'equipo', 'datetime', 'fase'}
...
def detectar_fichero():
    ...
    elif os.path.exists("datos_radio.csv"):
        return "datos_radio.csv", "/cansat/replay"
...
def limpiar_firebase(ruta):
    """Borra datos anteriores por lotes para evitar el límite de tamaño de Firebase."""
    try:
        r = requests.get(f"{FIREBASE_URL}{ruta}.json?shallow=true", timeout=10)
        claves = r.json()
        if not claves:
            print(f"🗑️  {ruta} ya estaba vacío")
        else:
            ...
            print(f"🗑️  Limpieza completada ({len(claves)} entradas borradas)")
...
def ejecutar_mision():
    ...
    if not archivo:
        ...
        print("   Sube 'datos_SD.csv', 'datos_radio.csv' o 'datos_simulacion.csv'")
        return
