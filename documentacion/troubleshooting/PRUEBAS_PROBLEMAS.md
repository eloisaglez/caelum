...
### Problema 9: Detección de fase errónea — CanSat en caída libre sin haber despegado
**Síntoma:**
```
fase = caida_libre  (con el CanSat en tierra, sin lanzar)
```

**Causa:** La función `actualizarFase()` comparaba solo el eje Z del acelerómetro:
```cpp
// ❌ Solo eje Z — falla si el CanSat está montado verticalmente
if (acZmG < ACCEL_CAIDA_LIBRE) { faseActual = CAIDA_LIBRE; }
```
Si el CanSat está montado verticalmente dentro de la carcasa (lo normal), la gravedad
se reparte entre los tres ejes y `acZ` puede ser ~0 mG aunque el CanSat esté en reposo.
El programa lo interpretaba como caída libre.

**Solución:** Usar la **magnitud total** del vector de aceleración (los 3 ejes combinados).
En reposo siempre hay ~1000 mG de magnitud total, independientemente de la orientación:
```cpp
// ✅ Magnitud total — funciona con cualquier orientación
#include <math.h>
float acMagmG = sqrt(acX*acX + acY*acY + acZ*acZ) * 1000.0;
if (acMagmG < ACCEL_CAIDA_LIBRE) { faseActual = CAIDA_LIBRE; }
```
La función también recibe los 3 ejes en lugar de solo `acZ`:
```cpp
// ANTES:
actualizarFase(altRel, acZ);
// AHORA:
actualizarFase(altRel, acX, acY, acZ);
```
Umbral de detección de tierra ampliado de ±50 a ±100 mG para mayor robustez.

**Versión corregida:** `CANSAT_VUELO_INTEGRADO_v3.ino`

---

### Problema 10: La grabación de datos no se activaba durante el vuelo real
**Síntoma:** Todos los registros del día del lanzamiento (Villanueva del Pardillo, 18 marzo 2026)
salieron en fase `tierra`. La grabación en RAM nunca se activó.

**Causa:** La grabación dependía de entrar en fase `DESCENSO`, que nunca se alcanzó
porque la detección de fases no funcionó correctamente (ver Problema 9):
```cpp
// ❌ Solo graba si el algoritmo de fases funciona correctamente
if (!grabandoRAM && faseActual == DESCENSO) { grabandoRAM = true; }
```
Si la detección de fases falla por cualquier motivo, no se graba nada aunque el CanSat
esté volando.

**Solución:** Activar la grabación por **altura barométrica** en lugar de por fase detectada.
Se añade un flag de modo al principio del programa — un único cambio de una palabra:
```cpp
// !! CAMBIAR SOLO ESTA LÍNEA ANTES DE SUBIR !!
#define MODO  LANZAMIENTO   // opciones: PRUEBA_2M | PRUEBA_7M | LANZAMIENTO
```

| Modo        | Altura de activación | Uso                             |
|-------------|----------------------|---------------------------------|
| PRUEBA_2M   | 2 m                  | Pruebas en mesa o escaleras     |
| PRUEBA_7M   | 7 m                  | Pruebas desde ventana o tejado  |
| LANZAMIENTO | 50 m                 | Competición                     |

Al arrancar, el monitor serial confirma el modo activo:
```
MODO: LANZAMIENTO
Grabacion activa desde: 50.0 m
```
La SD sigue grabando durante la fase `espera` en tierra como log de diagnóstico.
La grabación RAM se activa en cuanto se supera la altura configurada, sin depender
de la detección automática de fases.

**Versión corregida:** `CANSAT_VUELO_INTEGRADO_v3.ino`

---

### Problema 11: Timestamp en Firebase es un contador (0, 1, 2…) en vez de tiempo real
**Síntoma:** Los registros en Firebase tienen claves `0`, `1`, `2`… en lugar de fechas reales.
No es posible ordenar ni filtrar datos por tiempo real en Firebase.

**Causa:** El Arduino usa `timestamp` como contador simple desde el arranque:
```cpp
// Arduino — contador simple, NO tiempo real
timestamp++;  // 0, 1, 2, 3...
```
Y `receptor_telemetria.py` usaba ese valor como clave en Firebase:
```python
# ❌ Usa el contador del Arduino como clave
ts  = int(payload.get('timestamp', int(time.time())))
url = f"{FIREBASE_URL}{RUTA}/{ts}.json"
```

**Solución:** El receptor Python ignora el timestamp del Arduino y usa la hora real del PC:
```python
# ✅ Siempre usa tiempo Unix real del PC receptor
ts  = int(time.time())
url = f"{FIREBASE_URL}{RUTA}/{ts}.json"
```
El campo `timestamp` del CSV sigue siendo el contador del Arduino (útil para gráficas),
pero las claves en Firebase son siempre tiempos reales. Esto permite ordenar y filtrar
datos por hora en Firebase sin depender del reloj del Arduino.
...
| Claves Firebase son 0,1,2… en vez de hora real | `receptor_telemetria.py` usaba el contador del Arduino | Usar `int(time.time())` en el receptor Python |
...
### Programa principal
- **Archivo:** `CANSAT_VUELO_INTEGRADO_v3.ino`
- Todos los sensores integrados y externos
- Detección automática de fases de vuelo (magnitud 3 ejes — orientación independiente)
- Grabación activada por altura configurable (no depende de detección de fases)
- Almacenamiento en MicroSD (datos_SD_raw.csv, datos crudos) + backup RAM
- Telemetría via APC220

### Flag de modo — cambiar antes de cada sesión
```cpp
// Línea 93 del programa — CAMBIAR SOLO ESTA PALABRA
#define MODO  LANZAMIENTO   // PRUEBA_2M | PRUEBA_7M | LANZAMIENTO
```

| Modo        | Altura grabación | Uso                             |
|-------------|------------------|---------------------------------|
| PRUEBA_2M   | 2 m              | Pruebas en mesa o escaleras     |
| PRUEBA_7M   | 7 m              | Pruebas desde ventana o tejado  |
| LANZAMIENTO | 50 m             | Competición                     |
...
8. **La detección de fases necesita los 3 ejes** — Usar solo `acZ` para detectar caída libre falla cuando el CanSat está montado verticalmente. La magnitud total del vector `sqrt(acX²+acY²+acZ²)` funciona con cualquier orientación.

9. **La grabación no debe depender de la detección de fases** — Si el algoritmo de fases falla, no se graba nada. Activar la grabación por altura barométrica es más robusto y no depende de ningún otro subsistema.

10. **Un flag de modo evita errores en el lanzamiento** — Tener PRUEBA_2M / PRUEBA_7M / LANZAMIENTO como una sola línea visible arriba del programa elimina el riesgo de subir la configuración incorrecta el día del vuelo.

---

## ⚠️ Pendiente al Finalizar el Proyecto

### Restringir escritura en Firebase
Una vez terminada la competición, cambiar las reglas de Firebase para que solo sea posible leer datos públicamente pero no escribir sin autenticación. Esto evita que alguien externo pueda modificar o corromper la base de datos.

**Pasos:**
1. Ir a [Firebase Console](https://console.firebase.google.com/) → proyecto `cansat-66d98`
2. Realtime Database → Reglas
3. Cambiar las reglas actuales por:
```json
{
  "rules": {
    ".read": true,
...
Con esto el dashboard sigue funcionando (lectura pública) pero nadie puede escribir sin credenciales de administrador.
...
**Programa principal:** CANSAT_VUELO_INTEGRADO_v3.ino
