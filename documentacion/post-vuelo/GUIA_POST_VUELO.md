...
El CanSat guarda los datos en **dos lugares simultáneamente**:

| Fuente | Dónde | Cuándo usar |
|--------|-------|-------------|
| **MicroSD** | Tarjeta SD dentro del CanSat | **Siempre** — fuente principal |
| **RAM backup** | Memoria interna del Arduino | Si la SD falla — fuente de emergencia |

> ⚠️ La RAM backup se pierde si se apaga el Arduino. La SD card conserva los datos aunque la batería se agote.
...
4. Buscar el archivo **`VUELO.CSV`** en la raíz de la tarjeta
5. **Copiar** `VUELO.CSV` a tu ordenador
6. Renombrar para identificar el vuelo:
   ```
   VUELO.CSV → vuelo_brunete_17marzo.csv   (ejemplo)
   ```
...
Si `VUELO.CSV` no existe o está vacío, usar el backup de RAM.

### Material necesario:
- Cable USB
- Ordenador con Arduino IDE o PuTTY

### Pasos:

1. **Conectar** el CanSat al ordenador por USB (**sin apagarlo**)
2. **Abrir Arduino IDE** → Herramientas → Monitor Serie
3. **Configurar** a 115200 baud
4. **Escribir** `CSV_RAM` y pulsar Enter

   El Arduino responderá con algo así:
   ```
   --- CSV RAM: inicio ---
   t_ms,alt_m,co2_ppm,pm25,temp_hs,hum_hs,temp_scd,hum_scd,temp_lps,acX,acY,acZ,fase
   1000,950.2,421,6.1,5.8,68.2,5.9,67.0,6.2,0.02,-0.01,9.81,caida_libre
   2000,932.5,422,5.8,5.9,68.0,6.0,66.8,6.3,...
   ...
   --- FIN: 185 registros ---
   ```

5. **Seleccionar** todo el texto (desde la cabecera hasta `FIN`)
6. **Copiar** (Ctrl+C)
7. **Abrir Bloc de notas**, pegar y guardar como `vuelo_ram.csv`
...
> Para limpiar la RAM tras exportar: escribir `BORRAR_RAM` en el Monitor Serie.
...
cp vuelo_brunete_17marzo.csv software/post_vuelo/python/
...
1. Subir `caelum_datos_vuelo.csv` a Google Colab
...
```bash
# En Colab o local
python caelum_playback.py caelum_datos_vuelo.csv
```
...
> El CO₂ a 1000 m **no indica calidad del aire** — indica si la atmósfera está bien mezclada.

| CO₂ vs Altitud | Interpretación |
|----------------|----------------|
| ~420 ppm constante en todo el perfil | ✅ Atmósfera bien mezclada — normal |
| Aumenta gradualmente al bajar | Fuentes de combustión en superficie (tráfico) |
| Pico en una altitud concreta | Posible capa de inversión atrapando gases |
| Variación > 20 ppm entre altitudes | Capas atmosféricas diferenciadas detectadas |
...
### Firmas de combustión combinadas

```
CO₂ ALTO + PM2.5 ALTO  → Combustión activa (motor, fuego)
CO₂ ALTO + PM2.5 BAJO  → Fermentación/respiración (raro en exterior)
CO₂ BAJO + PM2.5 ALTO  → Polvo sin combustión (obra, viento)
CO₂ BAJO + PM2.5 BAJO  → Aire limpio ✓
```
...
|----------|---------------|----------|
| `VUELO.CSV` no existe | SD no se inicializó | Usar RAM backup (`CSV_RAM`) |
...
| `analizar_vuelo_v4.py` da error | Columnas incorrectas | Verificar cabecera del CSV con Excel |
...
[ ] SD card extraída
[ ] VUELO.CSV copiado al ordenador
[ ] VUELO.CSV verificado en Excel (25 columnas, datos coherentes)
[ ] Si SD vacía: RAM exportada con CSV_RAM y guardada
[ ] Script analizar_vuelo_v4.py ejecutado correctamente
